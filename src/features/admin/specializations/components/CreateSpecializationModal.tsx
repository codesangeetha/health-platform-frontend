...
import { createSpecialization } from '../../../../services/admin/specializations.service';
import { ApiError } from '../../../../services/auth/auth.service';
import type { CreateSpecializationRequest } from '../../../../types/specialization/specialization.types';
...
export function CreateSpecializationModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void; }) {
  const [form, setForm] = useState<CreateSpecializationRequest>({
    name: '',
  });
  ...
  const setField = (key: keyof CreateSpecializationRequest, value: any) => {
  ...
  const validateField = (name: keyof CreateSpecializationRequest, value: string): string | undefined => {
    switch (name) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    const key = name as keyof CreateSpecializationRequest;
    if (touched[key]) {
  ...
  return (
    <Backdrop open={open}>
      <Modal role="dialog" aria-modal="true" aria-label="Create Specialization">
        <Header>Create Specialization</Header>
        <Form onSubmit={handleSubmit}>
          ...
          <Actions>
            ...
            <Button type="submit" disabled={submitting}>{submitting ? 'Creating...' : 'Create Specialization'}</Button>
          </Actions>
        </Form>
      </Modal>
    </Backdrop>
