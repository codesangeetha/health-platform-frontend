...
import { deleteCategory } from '../../../../services/admin/pharmacy.service';
...
export function DeleteCategoryDialog({ open, category, onClose, onDeleted }: DeleteCategoryDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        setError(err?.message || 'Failed to delete medicine category');
      }
    } finally {
      setDeleting(false);
    }
  ...
  return (
    <Backdrop open={open}>
      <Dialog>
        <Header>Delete Medicine Category</Header>
        <Content>
          {error && (
            <div style={{
              background: '#ffeff0',
