...
import { updateLabTestCategory } from '../../../../services/admin/lab-test-categories.service';
import { ApiError } from '../../../../services/auth/auth.service';
import type { LabTestCategory, UpdateLabTestCategoryPayload } from '../../../../types/lab-test-category/lab-test-category.types';
...
interface EditLabTestCategoryModalProps {
  open: boolean;
...
export function EditLabTestCategoryModal({ open, category, onClose, onUpdated }: EditLabTestCategoryModalProps) {
    const [form, setForm] = useState<UpdateLabTestCategoryPayload>({});
    ...
    const setField = (key: keyof typeof form, value: any) => {
        setForm((prev: UpdateLabTestCategoryPayload) => ({ ...prev, [key]: value }));
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setForm((prev: UpdateLabTestCategoryPayload) => ({ ...prev, [name]: value }));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setSubmitting(true);
            const payload: UpdateLabTestCategoryPayload = {
                name: form.name?.trim(),
                description: form.description?.trim(),
                status: form.status,
            };

            await updateLabTestCategory(category.categoryId, payload);
            onUpdated();
        } catch (err: any) {
            if (err instanceof ApiError) {
                setError(err.message);
            } else {
                setError(err?.message || 'Failed to update lab test category');
            }
        } finally {
            setSubmitting(false);
        }
    ...
    return (
        <>
            <Backdrop open={open}>
                <Modal role="dialog" aria-modal="true" aria-label="Edit Lab Test Category">
                    <Header>Edit Lab Test Category</Header>
